      ...
      <CRow>
        <CCol md="12" class="mb-4">
          ...
          <div class="p-3 border rounded bg-light shadow-sm">
            <CFormInput label="ชื่อโครงการ (ภาษาไทย)" :value="form.titleTH"
              @input="updateField('titleTH', $event.target.value)" placeholder="กรุณาระบุชื่อโครงการภาษาไทย"
              class="mb-3" />
            <CFormInput label="ชื่อโครงการ (ภาษาอังกฤษ)" :value="form.titleEN"
              @input="updateField('titleEN', $event.target.value)" placeholder="Project Title in English"
              class="mb-0" />
          </div>
        </CCol>
          ...
          <div class="d-flex flex-wrap border rounded p-3 bg-white shadow-sm" style="gap: 20px;">
            <div v-for="type in budgetTypes" :key="type.value" class="custom-control custom-radio">
              <input class="custom-control-input" type="radio" :id="'type-' + type.value" :value="type.label"
                :checked="form.budgetType === type.label" @change="updateField('budgetType', type.label)">
